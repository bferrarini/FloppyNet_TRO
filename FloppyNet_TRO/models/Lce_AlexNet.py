...
class QuantizedAlexNet(models.abstract.ModelWrapper):
    ...
    def __init__(self, model_name, working_dir, nClasses, fc_units = 512, enable_history = False, activation_precision : int = 1, kernel_precision : int = 1, model_name_2 = "checkpoint", logger_lvl = config.log_lvl, l_rate = 0.0001, **kwargs):
        super(QuantizedAlexNet, self).__init__(model_name, working_dir, model_name_2, logger_lvl = logger_lvl, enable_history = enable_history,**kwargs)    
    ...
    def _setup_model(self, **kwargs):
        ...
        cnn = self._cnn(input_tensor = input_img,
                        input_precision=self.activation_precision,
    ...
    def _cnn(self,input_tensor=None, input_shape=None,
        ...
        #Block 3
        x = self._conv_block(384,  kernel_size = (3,3), strides = (1,1), name='conv3',
                                            input_quantizer = 'ste_sign',
                                            kernel_quantizer = 'ste_sign',
                                            kernel_constraint = lq.constraints.WeightClip(clip_value=kernel_clip),
                                            use_bias = use_bias,
                                            momentum = momentum)(x)
        x = self._conv_block(384, kernel_size = (3,3), strides = (1,1), name='conv4',
                                            input_quantizer = 'ste_sign',
                                            kernel_quantizer = 'ste_sign',
                                            kernel_constraint = lq.constraints.WeightClip(clip_value=kernel_clip),
                                            use_bias = use_bias,
                                            momentum = momentum)(x)
        x = self._conv_block(256, kernel_size = (3,3), strides = (1,1), name='conv5',
                                            input_quantizer = 'ste_sign',
                                            kernel_quantizer = 'ste_sign',
                                            kernel_constraint = lq.constraints.WeightClip(clip_value=kernel_clip),
                                            use_bias = use_bias,
    ...
    def get_inner_layer_by_name(self, layer_name, flatten=False, activation = None):
